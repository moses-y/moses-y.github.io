 ...
 * separately from the builder drifts from it, and test-relations.js asserts the two
...
const TOP = {
  ...
  links: ['array', 'Semantic similarity edges as [sourceId, targetId, similarity]. Undirected despite the ordering, cosine similarity in the embedding space, 3 decimal places. Both ids are `i` values in `repos`. INFERRED - carries no evidence beyond the score. The EXTRACTED counterpart, shared declared dependencies with the packages named, is in /data/kin/<id>.json.'],
...
function describe() {
  ...
  return {
    ...
    description: 'The read side of a deterministic analysis pipeline over ' +
      ...
      'the file small; this document is the key. Every field in a record is ' +
      'EXTRACTED - measured from the repository tree, its history or its ' +
      'manifests, with no model involved - with one exception: `u` is a ' +
      'projection of a neural embedding, and the `links` array derived from it ' +
      'is therefore INFERRED. See /data/relations.json for the provenance of ' +
      'every edge type.',
