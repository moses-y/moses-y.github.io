 ...
 *
 * Usage:
 *   node src/site/build-bundles.js            rebuild both
 *   node src/site/build-bundles.js --check     exit non-zero if either is stale
 */
'use strict';
const fs = require('fs');
...
function header(bundle) {
  ...
  const lines = [
    ...
    '',
    'Built by src/site/build-bundles.js from ' + src + '/',
...
if (CHECK && stale) {
  console.error('Run:  node src/site/build-bundles.js');
  process.exit(1);
}
