 ...
 * quietly rendering.
 *
 *   node src/stages/build-store.js            # core + reference data
 *   node src/stages/build-store.js --deep     # also per-repo modules and symbols
 *   node src/stages/build-store.js --verify   # counts against the JSON sources
 */
'use strict';

const fs = require('fs');
const path = require('path');
const { open, tx, bool } = require('../lib/lib-db.js');
...
function loadReference(db) {
  ...
  for (const f of fs.readdirSync(CHECKS_DIR).filter(n => /^checks-.*\.js$/.test(n))) {
